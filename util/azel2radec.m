function [r, d] = azel2radec(A, a, lat, lon, time)
% Programed by Darin C. Koblick 6/16/2009
%--------------------------------------------------------------------------
% Updated:                                                      Date:
% - changed input unit requirements: deg->rad                2/4/2016
...
% Inputs:                                                       Format:
%--------------------------------------------------------------------------
% Local Azimuth Angle   (rad)                               [N x 1]
% Local Elevation Angle (rad)                               [N x 1]
% Lat (Site Latitude in rad -90:90 -> S(-) N(+))            [N x 1]
% Lon (Site Longitude in rad -180:180 W(-) E(+))            [N x 1]
% UTC (Coordinated Universal Time dd-mmm-yyyy hh:mm:ss)          [N x 1]
%
% Outputs:                                                      Format:
%--------------------------------------------------------------------------
% Topocentric Right Ascension (rad)   [N x 1]
% Topocentric Declination Angle (rad)                       [N x 1]
%
%
...
% Convert everything to deg (rest of the function works in degs)
lat = rad2deg(lat);
lon = rad2deg(lon);
A = rad2deg(A);
a = rad2deg(a);
...
[yyyy mm dd HH MM SS] = datevec(datenum(time,'dd-mmm-yyyy HH:MM:SS'));
...
%Algorithm 28
d = asind(sind(a).*sind(lat)+cosd(a).*cosd(lat).*cosd(A));
LHA = atan2(-sind(A).*cosd(a)./cosd(d), ...
    (sind(a)-sind(d).*sind(lat))./(cosd(d).*cosd(lat))).*(180/pi);
% r = mod(ThetaLST-LHA,360);
r = ThetaLST-LHA;


% Convert everything back to rads
r = deg2rad(r);
d = deg2rad(d);
